...
Verification at each stage: `tsc --noEmit` + `lint` + `test` (124) + `build` (26 routes), plus no-SSR / FSD-layering / server-only / no-signing grep gates.

## Next steps

1. ~~**Stage 2b**~~ — **Done** (see above).
2. ~~**Stage 2c**~~ — **Done** (see above).
3. **Stage 3** — admin users (user management, role assignment, admin-only guards).
4. **Stage 4** — public viewer `/p/[publicId]` + access control; relocate shared preview components (`proposal-preview`, `fullscreen-slides`, `canvas-view`) + `variant-tabs` / `proposal-editor-preview` shells out of `src/legacy` (currently deferred here due to pins + public-viewer coupling).
