  ...
  return (
    <>
      <section className="space-y-3">
        ...
        <ul className="space-y-2">
          {active.versions.map((v) => (
            <li
              key={v.id}
              className="border-border flex items-center justify-between rounded-[8px] border px-4 py-2"
            >
              <span className="text-sm">
                v{v.versionNo}
                {v.note ? ` — ${v.note}` : ""}
                {v.id === active.currentVersionId && (
              ...
              <RestoreButton
                ...
                variantId={active.id}
                versionId={v.id}
                isCurrent={v.id === active.currentVersionId}
              />
            </li>
          ))}
        </ul>
      ...
      <section className="space-y-3">
        ...
        <div className="border-border h-[80vh] overflow-hidden rounded-[8px] border">
