    ...
    <div className="flex h-full w-full gap-4 overflow-x-auto p-4">
      {columns.map((col) => (
        <div key={col.slug} className="flex h-full min-w-[280px] flex-1 flex-col">
          ...
          <div className="border-border min-h-0 flex-1 space-y-3 overflow-y-auto rounded-[8px] border p-2">
            {col.pages.length === 0 ? (
              <p className="text-muted-foreground py-8 text-center text-xs">미리보기 없음</p>
            ) : (
              col.pages.map((pg) => (
