...
export function VariantViewerNav({ items, activeSlug, onList, onSelect, onCompare }: {
  items: NavItem[];
  activeSlug: string;
  onList: () => void;
  ...
  return (
    <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-border p-2">
      <button type="button" onClick={onList} className="text-sm underline">목록</button>
      <span className="text-muted-foreground">·</span>
      {items.map((it) => (
        <button key={it.slug} type="button" onClick={() => onSelect(it.slug)} className="cursor-pointer">
          <Badge variant={it.slug === activeSlug ? "default" : "outline"}>{it.label}</Badge>
        </button>
      ))}
      <span className="text-muted-foreground">·</span>
      <button type="button" onClick={onCompare} className="text-sm underline">나란히 보기</button>
    </div>
  );
}
