...
// 현재 ANALYSIS_VERSION 기준으로 아직 analyzed 되지 않은 페이지를 조회한다.
// 대상은 각 변형의 "현재 버전" 페이지(옛 버전은 제외). analyzed면 스킵, 없음/failed면 재분석 대상.
// onlyExposed=true면 exposed_to_uxisworks 시안만(대표 시안 위주 백필).
export async function listPendingPages(
  opts: { limit?: number; onlyExposed?: boolean } = {},
): Promise<PendingPage[]> {
  const { limit = 200, onlyExposed = false } = opts;
  ...
  for (const p of pages) {
    if (analyzedSet.has(p.pageId)) continue;
